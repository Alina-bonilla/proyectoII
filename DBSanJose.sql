...
AS
	...
	DECLARE	@CONSULTA NVARCHAR(900) = 'SELECT ProductoID,PROD.Nombre As Producto,CATE.Nombre AS Categoria,PROD.EnfermedadesRelacionadas,PROD.PrecioUnitario FROM DBSanJose.dbo.Productos PROD INNER JOIN DBSanJose.dbo.Categorias CATE ON CATE.CategoriaID = PROD.CategoriaID '
...
AS
	SELECT 
		...
		PROD.PrecioUnitario ,
		PROD.Foto
	FROM DBSanJose.dbo.Productos PROD 
	INNER JOIN DBSanJose.dbo.Categorias CATE ON CATE.CategoriaID = PROD.CategoriaID
...
AS
	...
	FROM DBSanJose.dbo.Categorias CATE
	LEFT JOIN DBSanJose.dbo.Productos PROD ON CATE.CategoriaID = PROD.CategoriaID
...
AS
	INSERT INTO DBSanJose.dbo.Productos (CategoriaID,Marca,Nombre,EnfermedadesRelacionadas,PrecioUnitario,Foto)
...
AS
	INSERT INTO DBSanJose.dbo.Categorias(Nombre)
	VALUES (@pNombre);
;
...
AS
	IF  EXISTS (SELECT * FROM DBSanJose.dbo.Productos where ProductoID = @pProductoID)
	BEGIN
		IF  EXISTS (SELECT * FROM DBSanJose.dbo.Inventarios where ProductoID = @pProductoID)
		BEGIN
			DECLARE	@SEVERALCOLUMNS BIT = 0;
			DECLARE	@CONSULTA NVARCHAR(700);
			SET @CONSULTA = 'UPDATE DBSanJose.dbo.Inventarios
			SET '
			IF @pCantidad > -1
			BEGIN
				SET @SEVERALCOLUMNS = 1
		...
		BEGIN
			...
			BEGIN
				INSERT INTO DBSanJose.dbo.Inventarios ([ProductoID], [Cantidad], [CostoUnitario])
...
AS 
BEGIN
    IF EXISTS (SELECT * FROM DBSanJose.DBO.Inventarios WHERE ProductoID = @pProductoID AND Cantidad >= @pCantidad) 
	BEGIN
	SELECT 1
        RETURN 1
    END
...
AS
	INSERT INTO DBSanJose.DBO.Facturas (SucursalID,Cliente,Fecha,Impuesto,Subtotal,Total)
    VALUES ('SaJo',@pCliente, GETUTCDATE(),@pImpuesto,0,0)
...
AS
	IF EXISTS (SELECT InventarioID FROM DBSanJose.DBO.Inventarios WHERE ProductoID = @pProductoID AND Cantidad >= @pCantidad)
	BEGIN
		DECLARE @PrecioUnitario	INT;
		SELECT @PrecioUnitario = PrecioUnitario FROM DBSanJose.DBO.Productos WHERE ProductoID = @pProductoID
...
AS	
	DECLARE	@SEVERALCOLUMNS BIT = 0;
	DECLARE	@CONSULTA NVARCHAR(1000) = 'SELECT FAC.FacturaID,FAC.Cliente,PROD.Nombre AS Producto,FAC.Fecha FROM DBSanJose.DBO.Facturas FAC INNER JOIN DBSanJose.DBO.DetallesFacturas DFA ON DFA.FacturaID = FAC.FacturaID INNER JOIN DBSanJose.DBO.Productos PROD ON PROD.ProductoID = DFA.ProductoID '
...
AS
	SELECT FAC.FacturaID,FAC.Cliente,FAC.Fecha,PROD.Nombre AS Producto, DFA.Cantidad,PROD.PrecioUnitario,DFA.Descuento,DFA.SubTotal,FAC.Impuesto,FAC.Total FROM DBSanJose.DBO.Facturas FAC
	INNER JOIN DBSanJose.DBO.DetallesFacturas DFA ON DFA.FacturaID = FAC.FacturaID
	INNER JOIN DBSanJose.DBO.Productos PROD ON PROD.ProductoID = DFA.ProductoID
...
EXECUTE DBO.ModificarInventario 1,50,2000;
EXECUTE DBO.ModificarInventario 2,65,1230;
EXECUTE DBO.ModificarInventario 3,40,975;
